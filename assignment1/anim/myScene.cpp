...
static int part2(ClientData clientData, Tcl_Interp* interp, int argc, myCONST_SPEC char** argv)
{
	...
	// retrieve the system
	BaseSystem* systemRetrieval;
	systemRetrieval = GlobalResourceManager::use()->getSystem("objectpath");
	assert(systemRetrieval);

	// register a simulator
	ObjectSimulator* objectSimulator = new ObjectSimulator("gravity", objectpath);
	success = GlobalResourceManager::use()->addSimulator(objectSimulator);
	assert(success);

	// retrieve the simulator
	BaseSimulator* simulatorRetrieval;
	simulatorRetrieval = GlobalResourceManager::use()->getSimulator("gravity");
	assert(simulatorRetrieval);
