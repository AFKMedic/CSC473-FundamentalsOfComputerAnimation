...
public class Spawner : MonoBehaviour
{
    ...
    void Update()
    {
        if (simulator_object.transform.childCount < max_particles)
        {
            ...
            particle.GetComponent<Particle>().velocity = new Vector3(   initial_x + Random.Range(-0.01f, 0.01f), 
                                                                        initial_y + Random.Range(-0.01f, 0.01f),
                                                                        initial_z + Random.Range(-0.01f, 0.01f));
